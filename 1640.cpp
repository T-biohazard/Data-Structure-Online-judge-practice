...
void build_dst(int ini)
{
    ...
    while (!pq.empty())
    {
        ...
        if (hotel[u])
            dst[hotel[ini]][hotel[u]] = c, newadj[hotel[ini]].pb(mp(hotel[u], c));
...
// dijsktra algo for finding shortest path
int dijsktra()
{
    ...
    while (!pq.empty())
    {
        ...
        int u = pq.top().S, c = -pq.top().F.F, t = -pq.top().F.S;
        pq.pop();
